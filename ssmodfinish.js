...
// @namespace    -
// @version     v6
// @description demonic cat picture is gonna eat u
// @author      me mega noob :( x scyfine
// @match        *://moomoo.io/*
...
give a credit just leave it here
*/
/*(function() {
    'use strict';
    ...
    function getkills(){
        ...
        if(count > kills){
            doNewSend(["6", [+ count + " kills, Pretty Easy!"]]);
        }
        kills = count;
    }
})();*/
...
document.getElementById("linksContainer2").style.display = "none", $("#mapDisplay").css({background: "url('https://i.imgur.com/fgFsQJp.png')"}), document.getElementById("adCard").remove(), document.getElementById("promoImgHolder").style.display = "none", window.onbeforeunload = null, window.follmoo();
document.addEventListener("keydown", function (e) {
    if (e.keyCode === 27) { // Escape key toggles the menu
        let menu = document.getElementById("mMDs");
        menu.style.display = menu.style.display === "none" ? "block" : "none";
    }
});

// Create the menu container
let mVDs = document.createElement("div");
mVDs.id = "mMDs";
document.body.prepend(mVDs);
...
Object.assign(menu.style, {
    ...
    color: "white",
    overflowY: "auto", // Only menu scrolls
    borderRadius: "25px",
    padding: "10px",
    zIndex: "9999"
});
...
menu.innerHTML = `
    <style>
        ...
        #mMDs { overflow-y: auto; max-height: 90vh; } /* Only menu can scroll */
    </style>
    <h3 style="font-size: 20px;" class="indent">Menu</h3>
    <label class="indent"><input id="autoBREAK" type="checkbox" checked/> Auto-Breaker</label><br>
    <label class="indent"><input id="autoPLACER" type="checkbox" /> Auto-Placer</label><br>
    <label class="indent"><input id="autoREPLACER" type="checkbox" /> Auto-Replacer</label><br>
    <label class="indent"><input id="clownCounter" type="checkbox" checked/> Shame Counter</label><br>
    <label class="indent"><input id="reloadBARSSSS" type="checkbox" /> Reload Bars</label><br>
    <label class="indent"><input id="buildHPez" type="checkbox" /> Building HP</label><br>
    <label class="indent"><input id="autoGrindez" type="checkbox" /> Auto-Grind</label><br>
    <label class="indent"><input id="aelookalike" type="checkbox" checked/> AE86 Action Bars</label><br>
    <label class="indent"><input id="hasGrid" type="checkbox" /> Grids</label><br>
    <label class="indent"><input id="qisop" type="checkbox" /> Auto Q</label><br>
    <label class="indent"><input id="showownerbuild" type="checkbox" /> Show Building Ownership</label><br>
    <label class="indent"><input id="predictAB" type="checkbox" /> pAB</label><br>
    <label class="indent"><input id="doEMPantis" type="checkbox" /> Do EMP Anti</label><br>
    <label class="indent"><input id="doSyncs" type="checkbox" /> Teammate Sync</label><br>
    <label class="indent"><input id="statusSucks" type="checkbox" /> Insta/Mill Status</label><br>
    <label class="indent"><input id="spiketick" type="checkbox" /> SpikeTick</label><br>
    <label class="indent"><input id="tickinstapro" type="checkbox" /> 2tickInsta</label><br>
    <label class="indent"><input id="Anti2Tick" type="checkbox" /> Anti2Tick</label><br>
    <label class="indent"><input id="wasdmode" type="checkbox" /> Wasd Mod (broken)</label><br>
    <label class="indent"><input id="kmtick1" type="checkbox" /> Katana & Musket Tick</label><br>
    <label class="indent"><input id="fzAutoHeal" type="checkbox" /> Fz Auto Heal</label><br>
    <label class="indent"><input id="noClownAutoQ" type="checkbox" /> No Clown AutoQ</label><br>
    <label class="indent"><input id="togglenightday" type="checkbox" /> Theme</label><br>

    <br>
    <div class="indent">
        <select id="spamType" width="70px">
            <option value="0" selected>Taking Over - LoL</option>
            <option value="1">Dead of Night - NCS</option>
            <option value="2">Walls - Rival</option>
            <option value="3">Don't Stand So Close - Initial D</option>
            <option value="4">Live A Lie - Rival</option>
        </select>
        <button id="startSpaming">Start Chat</button>
    </div>
    <div class="indent">
        <button id="toggleNightMode">Toggle Night Mode</button>
        <input type="checkbox" id="yesshadows" />
    </div>
    <br>
    <div class="indent">This is me mega noob's mod :0</div>
`;
 ...
 */
   ...
   , function(e, t, n) {
       (function(t) {
           e.exports.maxScreenWidth = 1920*1,
               e.exports.maxScreenHeight = 1080*1,
...
window.WebSocket = class {
        ...
        if (error) {
           ft(error);
            ...
            Ne.onclick = s.checkTrusted((function() {
                       (getElement("doSyncs").checked && R.team) && teamSync()
            }
            )),
    ...
    document.getElementById("pingDisplay")), Ee = document.getElementById("shutdownDisplay"), Me = document.getElementById("menuCardHolder"), Ae = document.getElementById("guideCard"), Pe = document.getElementById("loadingText"), Be = document.getElementById("gameUI"), Ce = document.getElementById("actionBar"), Oe = document.getElementById("scoreDisplay"), Re = document.getElementById("foodDisplay"), je = document.getElementById("woodDisplay"), _e = document.getElementById("stoneDisplay"), Ue = document.getElementById("killCounter"), De = document.getElementById("leaderboardData"), Le = document.getElementById("nameInput"), Fe = document.getElementById("itemInfoHolder"), ze = document.getElementById("ageText"), He = document.getElementById("ageBarBody"), Ve = document.getElementById("upgradeHolder"), qe = document.getElementById("upgradeCounter"), Ye = document.getElementById("allianceMenu"), We = document.getElementById("allianceHolder"), Xe = document.getElementById("allianceManager"), Ne = document.getElementById("mapDisplay"), Ge = document.getElementById("diedText"), Je = document.getElementById("skinColorHolder"), Ke = Ne.getContext("2d");
       ...
       function Yt(e, t) {
           ...
           Vt || (Vt = new function() {
               this.init = function(e, t) {
                  ...
                  qt.push(Vt)),
               Vt.init(e, t);
           if(getElement("doSyncs").checked && R.weapons[1] == 15 && R.skins[53] && isSyncTeam == false && R.team) {
               autoaim = true;
               isSyncTeam = true;
               eHat(53);
               r.send("K", 1);
       ...
       let ppeepee = document.querySelector("#aelookalike");
       ppeepee.addEventListener('change', function() {
           $t();
       });
       function $t(e, t, c) {
           ...
           for (var n = 0; n < l.list.length; ++n) {
               ...
               if(getElement("aelookalike").checked) {
                   document.getElementById("actionBarItem" + i).style.display = R.items.indexOf(l.list[n].id) >= 0 ? (R.items.indexOf(l.list[n].id) < 4) ? "inline-block" : "none" : "none"
                   if(getElement("aelookalike").checked) {
                       if(R.items[0] > 0 && R.items[0] < 3) {
                           if(R.items[0] == 1) {
                               document.getElementById("actionBarItem17").style.backgroundImage = "url('data:image/png;base64,')";
                           }else {
                               document.getElementById("actionBarItem18").style.backgroundImage = "url('data:image/png;base64,')";
                           }
                       }
                       if(R.items[2] > 6 && R.items[2] < 10) {
                           if(R.items[2] == 7) {
                               document.getElementById("actionBarItem23").style.backgroundImage = "url('data:image/png;base64,')";
                           }else if(R.items[2] == 8) {
                               document.getElementById("actionBarItem24").style.backgroundImage = "url('data:image/png;base64,')";
                           }else {
                               document.getElementById("actionBarItem25").style.backgroundImage = "url('data:image/png;base64,')";
                           }
                       }
                   }
               }else {
                   document.getElementById("actionBarItem17").style.backgroundImage = "url('data:image/png;base64,')",
                       document.getElementById("actionBarItem18").style.backgroundImage = "url('data:image/png;base64,')",
                       document.getElementById("actionBarItem23").style.backgroundImage = "url('data:image/png;base64,')",
                       document.getElementById("actionBarItem24").style.backgroundImage = "url('data:image/png;base64,')",
                       document.getElementById("actionBarItem25").style.backgroundImage = "url('data:image/png;base64,')",
       ...
       var nEy, nEA, nEs, autoaim = false, intrap = false, primaryReload = [], secondaryReload = [], turretReload = [], breakingBuild=false, wasdMills=false;
       var weaponON = null;
       var tickQueue = [], tickNum = 0;
       function setTickout(e, t) {
           "function" == typeof e &&
               ("object" != typeof tickQueue[tickNum + t] ?
                tickQueue[tickNum + t] = [e] : tickQueue[tickNum + t].push(e));
       }
       ...
       function buttonPressD(e) {
           if(e.button == 0) {
               if(isElementVisible(document.getElementById('enterGame'))==false) {
                   breakingBuild=!breakingBuild;
               }
           }
       }
       const autogriiind = document.querySelector("#autoGrindez");
       autogriiind.addEventListener('change', function() {
           if (this.checked) {
               for(let i=0;i<12;i++) {
                   Sn((R.items[5])?R.items[5]:R.items[3]), r.send("F", 1, toRad(30*i)), r.send("F", 0, toRad(30*i)), Sn(weaponON, 1);
               }
           }
       });
       var nightModeDay = {
       ...
       getElement("togglenightday").onclick = function() {
           if(nightModeDay.on == false) {
               ...
               getElement("togglenightday").innerHTML = "Toggle Day Mode";
           }else {
               for(let i = 0; i < 100; i++) {
                   setTimeout(() => {
                       nightModeDay.trans -= 0.003;
               ...
               getElement("togglenightday").innerHTML = "Toggle Night Mode";
           }
       }
       function place(id, angle = pn()) {
           Sn(id), r.send("F", 1, angle), r.send("F", 0, angle), Sn(weaponON, 1);
       }
       var overrideHatCustom = false;
       function eHat(id, override) {
           if(overrideHatCustom == false) {
               if(id > 0 && R.skins[id]) {
                   r.send("c", 0, id, 0);
               }else {
                   r.send("c", 0, 0, 0);
       ...
       function pn() {
           if(autoaim == false && intrap==false) {
               ...
               return nEA
           }else if(intrap==true && autoaim == false) {
               return trapAngle
           }
       }
       function fuck_pn() {
...
e23.addEventListener("mouseup", n2, false);
       ...
       var repeater = function(key, action, interval) {
           let _isKeyDown = false;let _intervalId = undefined;
           return {
               start(keycode) {
                   if(keycode == key && document.activeElement.id.toLowerCase() !== 'chatbox' && R.alive) {
                       _isKeyDown = true;
       ...
       function startSpam(e) {
           if(e == 0) {
               ...
               spamend = setTimeout(() => {
                   chatintervalss = [];
                   spamChating=false;
                   getElement("startSpaming").innerHTML = "Start Chat";
               }, takingover[takingover.length-1].delay);
           }else if(e == 1) {
               let deadofnight=[{chat:"Baby, this is do or die",delay:6278},{chat:"Feel it in my veins at night",delay:8013},{chat:"Emotional suicide",delay:10040},{chat:"You know it's an eye for eye",delay:12105},{chat:"I didn't wanna walk",delay:14580},{chat:"didn't wanna walk the plank",delay:16136},{chat:"No, but then ready or not",delay:19504},{chat:"then ready or not it came",delay:21679},{chat:"Like the thunder",delay:23864},{chat:"I was on my way",delay:25449},{chat:"to going under (under)",delay:26471},{chat:"Swimming in the pain",delay:28027},{chat:"yeah, I was covered",delay:29078},{chat:"In a tidal wave",delay:30472},{chat:"in a tidal wave",delay:32483},{chat:"But I'm a fighter (hu)",delay:34441},{chat:"Tryna take me down",delay:36035},{chat:"I'm going higher (I'm higher)",delay:37270},{chat:"Baby, you've",delay:38753},{chat:"been playing with some fire",delay:39645},{chat:"You've been playing with fire",delay:41269},{chat:"One day you will see",delay:43395},{chat:"What you made of me",delay:46593},{chat:"Found my inner beast",delay:49252},{chat:"You'll watch it release",delay:52018},{chat:"In thе dead of night",delay:54615},{chat:"In the dead of night",delay:65702},{chat:"In the dead of",delay:71539},{chat:"Night",delay:74635},{chat:"Baby, when it's do or die",delay:78883},{chat:"You know it's an eye for eye",delay:81419},{chat:"Feel the energy align (oh)",delay:84236},{chat:"In the dead of night",delay:86840},{chat:"In the dead of night",delay:89386},{chat:"In the dead of night",delay:95682},{chat:"In the dead of night",delay:98108},{chat:"You can save your alibi",delay:110718},{chat:"I already know you lied",delay:113044},{chat:"Oh no, no don't even try",delay:115969},{chat:"Watch the flame in me ignite",delay:118794},{chat:"You didn't wanna walk",delay:121519},{chat:"didn't wanna walk the plank",delay:122549},{chat:"But then ready or not",delay:126316},{chat:"then ready or not it came",delay:128043},{chat:"Baby, it was dark",delay:130130},{chat:"It was hard to see",delay:131452},{chat:"And that's when a spark",delay:132765},{chat:"lit inside of me, oh",delay:134036},{chat:"I was lost in reverie",delay:137831},{chat:"oh-oh, oh-oh",delay:141418},{chat:"One day you will see",delay:142629},{chat:"What you made of me",delay:145263},{chat:"What's inside of me",delay:148066},{chat:"Oh, one day you will see",delay:151204},{chat:"I found my inner beast",delay:155906},{chat:"You'll watch it release",delay:158579},{chat:"In the dead of night, oh",delay:161323},{chat:"In the dead of night",delay:172405},{chat:"In the dead of...",delay:174866},{chat:"In the dead of night, oh-woah",delay:177399},{chat:"In the dead of night",delay:180572},{chat:"Baby, when it's do or die",delay:185724},{chat:"You know it's an eye for eye",delay:188107},{chat:"Feel the energy align",delay:190628},{chat:"In the dead of night",delay:192998},{chat:"In the dead of night",delay:196522},{chat:"In the dead of night",delay:199153},{chat:"In the dead of night",delay:201643},{chat:"In the dead of night",delay:203893},{chat:"And one day you will see",delay:206604},{chat:"What you made of me",delay:209286},{chat:"What's inside of me",delay:212028},{chat:"Oh, and one day you will see",delay:215617},{chat:"I found my inner beast",delay:219619},{chat:"And you'll watch it release",delay:222411}];
               deadofnight.forEach(e => {
                   chatintervalss.push(setTimeout(() => {
                       if(document.activeElement.id.toLowerCase() !== 'chatbox') {
                           r.send("6", e.chat);
                       }
               ...
               spamend = setTimeout(() => {
                   ...
                   spamChating=false;
                   getElement("startSpaming").innerHTML = "Start Chat";
               }, deadofnight[deadofnight.length-1].delay);
           }else if(e == 2) {
               let wallsNCS=[{chat:"I was ready to surrender",delay:2e3},{chat:"my heart to you, only you",delay:6e3},{chat:"These lights all around me",delay:13e3},{chat:"Keep blinding my eyes from you",delay:15e3},{chat:"and the truth",delay:2e4},{chat:"Don't tell me, don't tell me",delay:24e3},{chat:"don't tell me why",delay:25e3},{chat:"I know, I know no you won't",delay:27e3},{chat:"save my life, save my life",delay:28e3},{chat:"If you keep on, you keep on",delay:34e3},{chat:"you making the waves",delay:36e3},{chat:"You and I will never be sane,",delay:37e3},{chat:"be sane",delay:42e3},{chat:"If these walls come down",delay:45e3},{chat:"Will you pick me",delay:5e4},{chat:"off the ground",delay:52e3},{chat:"If these walls come down",delay:55e3},{chat:"Will you save me or",delay:6e4},{chat:"burn me down",delay:62e3},{chat:"I was ready to surrender",delay:109e3},{chat:"my heart to you, only you",delay:113e3},{chat:"These lights all around me",delay:12e4},{chat:"Keep blinding my eyes from you",delay:122e3},{chat:"and the truth",delay:127e3},{chat:"Don't tell me, don't tell me",delay:131e3},{chat:"don't tell me why",delay:132e3},{chat:"I know, I know no you won't",delay:134e3},{chat:"save my life, save my life",delay:135e3},{chat:"If you keep on, you keep on",delay:141e3},{chat:"you making the waves",delay:143e3},{chat:"You and I will never be sane,",delay:144e3},{chat:"be sane",delay:149e3},{chat:"If these walls come down",delay:152e3},{chat:"Will you pick me",delay:157e3},{chat:"off the ground",delay:159e3},{chat:"If these walls come down",delay:162e3},{chat:"Will you save me or",delay:167e3},{chat:"burn me down",delay:169e3}];
               wallsNCS.forEach(e => {
                   chatintervalss.push(setTimeout(() => {
                       if(document.activeElement.id.toLowerCase() !== 'chatbox') {
                           r.send("6", e.chat);
               ...
               spamend = setTimeout(() => {
                   ...
                   spamChating=false;
                   getElement("startSpaming").innerHTML = "Start Chat";
               }, standsoclose[standsoclose.length-1].delay);
           }else if(e == 4) {
               let livealie=[{chat:"Took a swing at a wrecking",delay:3e3},{chat:"ball, and",delay:5e3},{chat:"I prayed for my downfall, when",delay:6e3},{chat:"I found no way to reconcile",delay:9e3},{chat:"'Cause in my heart",delay:12e3},{chat:"it's not worthwhile",delay:13e3},{chat:"Its a bloody battlefield where",delay:16e3},{chat:"Some go down, other's heal",delay:19e3},{chat:"In the end it's all the same",delay:21e3},{chat:"All you can do",delay:24e3},{chat:"is play the game",delay:25e3},{chat:"Oh, do we say goodbye?",delay:28e3},{chat:"Refuse to question why?",delay:32e3},{chat:"I'm too sad to say I'm sorry",delay:36e3},{chat:"So lie",delay:4e4},{chat:"pretend that you're okay",delay:41e3},{chat:"Swear that you will stay",delay:44e3},{chat:"Keep trying for one day",delay:49e3},{chat:"I'm sick of all these lies",delay:54e3},{chat:"Don't care if I capsize",delay:56e3},{chat:"There's nothing",delay:6e4},{chat:"left for me to hide",delay:61e3},{chat:"Don't wanna live a lie",delay:63e3},{chat:"Too many lows, too many highs",delay:65e3},{chat:"Too many things to criticize",delay:69e3},{chat:"And so I leave it all behind",delay:72e3},{chat:"Don't wanna live a lie",delay:75e3},{chat:"Life kept spitting",delay:102e3},{chat:"at my feet and",delay:103e3},{chat:"Forced me down",delay:104e3},{chat:"a dead end street when",delay:105e3},{chat:"All I had went up in flames",delay:108e3},{chat:"Burnin', only dark remains",delay:111e3},{chat:"I cover up every",delay:114e3},{chat:"part of my skin, oh",delay:115e3},{chat:"'Cause' I don't like",delay:117e3},{chat:"the person within, no",delay:118e3},{chat:"I know I'm the one to blame",delay:12e4},{chat:"All I can do is take the shame",delay:123e3},{chat:"Oh, do we say goodbye?",delay:125e3},{chat:"Refuse to question why?",delay:131e3},{chat:"I'm too sad to say I'm sorry",delay:135e3},{chat:"So lie",delay:138e3},{chat:"and pretend that you're ok",delay:14e4},{chat:"Swear that you will stay",delay:143e3},{chat:"Keep trying for one day",delay:148e3},{chat:"Whoa, I'm sick",delay:152e3},{chat:"of all these lies",delay:152500},{chat:"Don't care if I capsize",delay:156e3},{chat:"There's nothing",delay:158e3},{chat:"left for me to hide",delay:159e3},{chat:"Don't wanna live a lie",delay:162e3},{chat:"Too many lows, too many highs",delay:164e3},{chat:"Too many things to criticize",delay:167e3},{chat:"And so I leave it all behind",delay:17e4},{chat:"Don't wanna to live a lie",delay:173e3},{chat:"A lie",delay:178e3},{chat:"I don't wanna live a lie",delay:186e3},{chat:"Don't want",delay:19e4},{chat:"Live a lie, ooh, yeah",delay:193e3},{chat:"I'm sick of all these lies",delay:201e3},{chat:"Don't care if I capsize",delay:205e3},{chat:"Oh no",delay:209e3}];
               livealie.forEach(e => {
                   chatintervalss.push(setTimeout(() => {
                       if(document.activeElement.id.toLowerCase() !== 'chatbox') {
                           r.send("6", e.chat);
               ...
               spamend = setTimeout(() => {
                   ...
                   spamChating=false;
                   getElement("startSpaming").innerHTML = "Start Chat";
               }, livealie[livealie.length-1].delay);
           }else {
               return;
           }
       }
       var lmfao = [];
       getElement("startSpaming").onclick = function() {
           if(spamChating == true) {
               clearTimeout(spamend);
               getElement("startSpaming").innerHTML = "Start Chat";
               for(let i = 0; i < chatintervalss.length; i++) {
                   clearTimeout(chatintervalss[i]);
               }
               lmfao.push(setTimeout(() => {
                   chatintervalss = [];
               }, 500));
               if(lmfao.length > 1) {
                   clearTimeout(lmfao[0]);
                   lmfao.shift();
               }
           }else {
               getElement("startSpaming").innerHTML = "Stop Chat";
               startSpam(getElement("spamType").value);
           }
           spamChating=!spamChating;
       }
       var autoMillVar = {
           w: false,
           a: false,
       ...
       var autoPushToggle = false;
       function teamSync() {
           r.send("S", 1);
       }
       window.addEventListener("keydown", s.checkTrusted((function(e) {
           var t = e.which || e.keyCode || 0;
           tpPlacer.start(e.keyCode);foodPlacer.start(e.keyCode);spawnPlacer.start(e.keyCode);spikePlacer.start(e.keyCode);boostPlacer.start(e.keyCode);
           78 == t && "chatbox" !== document.activeElement.id.toLowerCase() &&
               1 == isElementVisible(getElement("gameUI")) &&
               (wasdMills = !wasdMills);
           if(t == 190 && "chatbox" !== document.activeElement.id.toLowerCase() && getElement("doSyncs").checked && R.team) {
               teamSync();
           }
           16 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (autoPushToggle = true);
           87 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (autoMillVar.w = true);
           65 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (autoMillVar.a = true);
           83 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (autoMillVar.s = true);
           68 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (autoMillVar.d = true);
           27 == t ? Qt() : R && R.alive && kn() && (gn[t] || (gn[t] = 1,
                                                               69 == t ? r.send("K", 1) : 67 == t ? (Mt || (Mt = {}),
                                                                                                     Mt.x = R.x,
        ...
        vn())))
           ...
           if (R && R.alive) {
               tpPlacer.stop(e.keyCode);foodPlacer.stop(e.keyCode);spawnPlacer.stop(e.keyCode);spikePlacer.stop(e.keyCode);boostPlacer.stop(e.keyCode);
               var t = e.which || e.keyCode || 0;
               16 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (autoPushToggle = false);
               87 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (autoMillVar.w = false);
               65 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (autoMillVar.a = false);
               83 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (autoMillVar.s = false);
               68 == t && "chatbox" !== document.activeElement.id.toLowerCase() && (autoMillVar.d = false);
       ...
       function bn() {
           ...
           (null == wn || null == e || Math.abs(e - wn) > .3) && (r.send("9", (getElement('autoGrindez').checked) ? null : e), wn = e)
       }
       var InstaVarible=false;
       function xn() {
           InstaVarible = !InstaVarible;
       }
       function Sn(e, t) {
           r.send("z", e, t)
       }
       function Tn() {
           I("moo_name", Le.value), !le && r.connected && (le = !0, x.stop("menu"), dt("Loading..."), r.send("M", {name: Le.value, moofoll: H, skin: ae}))
       }
       var In = !0;
       function En(e) {
           Pe.style.display = "none",
       ...
       var trapAngle, isabisyesorno=false, clearedTeam = true;
       ...
       function Cn(e) {
           ...
           let wEiT = traps.some(i => nEy.length && Math.hypot(i.y - nEy[2], i.x - nEy[1]) < 60);
           if(!getElement("autoGrindez").checked) {
               if(getElement("autoREPLACER").checked && Math.hypot(b.y - R.y2, b.x - R.x2) < 400) {
                   if(nEy.length && fgd(nEy, R) < 200) {
                       if(wEiT && fgd(nEy, R) < 150) {
                           if(R.primaryDamage >= 44) {
                               Sn(R.items[2]), r.send("F", 1, nEA), r.send("F", 0, nEA), Sn(weaponON, 1);
                           }else {
                               Sn(R.items[4]), r.send("F", 1, nEA), r.send("F", 0, nEA), Sn(weaponON, 1);
                           }
                       }
                       for(let i = 0;i < 24; i++){
                           let a = i*4*(i%2?-1:1)/180*Math.PI+nEA;
                           Sn(R.items[2]), r.send("F", 1, a), r.send("F", 0, a), Sn(weaponON, 1);
                       }
                   }else {
                       for(let i = 0;i < Math.PI*2;i += Math.PI/12){
                           Sn(R.items[4]), r.send("F", 1, i), r.send("F", 0, i), Sn(weaponON, 1);
                       }
                   }
               }
           }else {
               if(Math.hypot(b.y - R.y2, b.x - R.x2) < 400) {
                   for(let i = 0; i < 360; i+=15) {
                       Sn(R.items[5] ? R.items[5] : R.items[3]), r.send("F", 1, i), r.send("F", 0, i), Sn(weaponON, 1);
                   }
               }
           }
           nt.disableBySid(e);
               ...
               for (n = 0; n < _n.length; n++)
                   !function(e) {
                       ...
                       t.onmouseover = function() {
                           ...
                           if(getElement("aelookalike").checked) {
                               if(e == 17) {
                                   document.getElementById("actionBarItem" + e).style.backgroundImage = "url('data:image/png;base64,')";
                               }else if(e == 18) {
                                   document.getElementById("actionBarItem" + e).style.backgroundImage = "url('data:image/png;base64,')";
                               }else if(e == 23) {
                                   document.getElementById("actionBarItem" + e).style.backgroundImage = "url('data:image/png;base64,')";
                               }else if(e == 24) {
                                   document.getElementById("actionBarItem" + e).style.backgroundImage = "url('data:image/png;base64,')";
                               }else if(e == 25) {
                                   document.getElementById("actionBarItem" + e).style.backgroundImage = "url('data:image/png;base64,')";
               ...
               (_ = N[a]).active && (r = _.x + _.xWiggle - t,
                                     ...
                                     _.layer == e && ki(r, s, _.scale + (_.blocker || 0)) && (be.globalAlpha = _.hideFromEnemy ? .6 : 1,
                                                                                              _.isItem ? (i = ri(_),
                                                                                                          ...
                                                                                                          nightModeDay.on == true && getElement("yesshadows").checked && (be.shadowColor = 'black', be.shadowBlur = 5),
                                                                                                          be.drawImage(i, -i.width / 2, -i.height / 2),
                                                                                                          Math.hypot(_.x - R.x, _.y - R.y) < 500 && _.active && getElement("showownerbuild").checked && ["spikes", "poison spikes", "greater spikes", "spinning spikes", "pit trap"].includes(_.name) &&
                                                                                                          (be.globalAlpha = 1,
                                                                                                           be.fillStyle = _.owner ? _.owner.sid == R.sid ? "#00000000" : teamDetect(_.owner.sid) ? "#00000000" : "#cc5151" : "#00000000",
                                                                                                           si(0, 0, _.scale/4, be, true, false),
                                                                                                           be.strokeStyle = _.owner ? _.owner.sid == R.sid ? "#00000000" : teamDetect(_.owner.sid) ? "#00000000" : rt : "#00000000",
                                                                                                           si(0, 0, _.scale/4, be, false, true)),
       ...
       function Wn(e, t, i) {
           let o;
           (o = Ii(e)) && (hitde(configerdam(e, i)), o.startAnim(t, i), i < 9 ? setTimeout(() => {
               primaryReload[e] = 0;
           }) : setTimeout(() => {
               secondaryReload[e] = 0;
           }));
       ...
       function Xn(e, t, n) {
           ...
           for (var i = 0; i < W.length; ++i)
               (_ = W[i]).zIndex == n && (_.animate(P),//L = (!getElement("bigboyaime").checked ? _ == R ? fuck_pn() : _.dir : _.dir) + _.dirPlus
                                          _.visible && (_.skinRot += .002 * P,
                                                        L = (_ == R ? fuck_pn() : _.dir) + _.dirPlus,
                                                        be.save(),
       ...
       var ii = [];
       function ri(e, t) {
           var n = ii[e.id + (R && e.owner && e.owner.sid == R.sid ? 0 : (R && R.team && e.owner && teamDetect(e.owner.sid) ? 25 : 50))];
           if (!n || t) {
               ...
               n = i,
                   t || (ii[e.id + (R && e.owner && e.owner.sid == R.sid ? 0 : (R && R.team && e.owner && teamDetect(e.owner.sid) ? 25 : 50))] = n)
           }
           return n
       }
       ...
       var resALL = [];
       function hitde(e){
           for(let i = 0; i < resALL.length; i++){resALL[i](e)}
           resALL = [];
       }
       function damagerrr(){
           return new Promise(function(resolve, reject){resALL.push(resolve);setTimeout(function(){reject();},50)})
       }
       function plAntiTrap(e, t) {
           let trapAngles = Math.atan2(e-R.x2, t-R.y2);
           for(let i=0;i<12;i++) {
               let angleFOrmal=trapAngles+toRad(180+i*30);
               Sn(R.items[4]), r.send('F', 1, angleFOrmal), r.send('F', 0, angleFOrmal), r.send('z', weaponON, 1);
           }
       }
       function li(e) {
           for (var t = 0; t < e.length; )
               nt.add(e[t], e[t + 1], e[t + 2], e[t + 3], e[t + 4], e[t + 5], l.list[e[t + 6]], !0, e[t + 7] >= 0 ? {
                   ...
                   (15==e[t+6]&&(R.x2-e[t+1])**2+(R.y2-e[t+2])**2<1e4&&e[t+7]!=R.sid&&!teamDetect(e[t+7])) && plAntiTrap(e[t + 1], e[t + 2]), t += 8
       }
       function hi(e, t) {
           let _;(_ = Mi(t)) && (_.xWiggle += o.gatherWiggle * Math.cos(e), _.yWiggle += o.gatherWiggle * Math.sin(e), (damagerrr().then(function(l){_.curretHealth -= l}).catch(function(e){resALL = [];})))
       }
       function ui(e, t) {
       ...
       function fi(e, t, n, i, r, s, a, o) {
           lt && (J.addProjectile(e, t, n, i, r, s, null, null, a).sid = o, function(e, t, n, i, r) {
               ...
               for (let i = 0; i < W.length; i++) {
                   (_ = W[i]) && _.visible && _.secondary && l.weapons[_.secondary].projectile !== undefined && l.projectiles[l.weapons[_.secondary].projectile].speed == r && min > (_.x2 * 1.5 - _.x1 / 2 - e + Math.cos(n) * 80) ** 2 + (_.y2 * 1.5 - _.y1 / 2 - t + Math.sin(n) * 80) ** 2 && (id = _.sid, min = (_.x2 * 1.5 - _.x1 / 2 - e + Math.cos(n) * 80) ** 2 + (_.y2 * 1.5 - _.y1 / 2 - t + Math.sin(n) * 80) ** 2);
               }
               if (Math.sqrt(min) > 60) {
                   if (r == 1.5) {
                       for (let i = 0; i < W.length; i++) {
                           (_ = W[i]) && _.visible && min > (_.x2 * 1.5 - _.x1 / 2 - e + Math.cos(n) * 10) ** 2 + (_.y2 * 1.5 - _.y1 / 2 - t + Math.sin(n) * 10) ** 2 && (sid = _, id = _.sid, min = (_.x2 * 1.5 - _.x1 / 2 - e + Math.cos(n) * 10) ** 2 + (_.y2 * 1.5 - _.y1 / 2 - t + Math.sin(n) * 10) ** 2);
                       }
                       if (Math.sqrt(min) < 60) {
                           turretReload[id] = -0.0444;
                       }
                   }else {
                       secondaryReload[id] = 0;
                   }
               } else {
                   _ = Ii(id);
       ...
       var normalh = 0, appleh = 0, antibullh = 0;
       function heal(e, delay, tickBased) {
           let foodType = (R.items[0] == 0 ? 20 : R.items[0] == 1 ? 40 : 30)
           let times = (e == "max" ? (100-R.health)/foodType : e == (null || undefined) ? 1 : e);
           if(tickBased) {
               setTickout(() => {
                   for(let i = 0; i < times; i++) {
                       Sn(R.items[0]), r.send("F", 1, pn()), r.send("F", 0, pn()), Sn(weaponON, 1);
                   }
               }, delay);
           }else {
               setTimeout(() => {
       ...
       function instaheal(e, t) {
           ...
           if(getElement("doEMPantis").checked && R.skins[22]) {
               overrideHatCustom = true;
               eHat(22, true);
               setTickout(() => {
                   overrideHatCustom = false;
               }, 2);
           }
       }
       function Si(e, a) {
           if((_ = Ii(e)) && (function(_, t) {
               let d = t - _.health;
               if(d > 0){
           ...
           }(_, a), _.lasHealth = _.health, _.health = a), e == R.sid) {
               let damage = R.lasHealth - R.health;
               if(getElement("predictAB").checked) {
                   [60, 66, 71].map(e => Math.round(e * (6 == R.skinIndex ? .75 : 1))).includes(damage) &&
                       (pAB || pAB2 ? (
                       pAB = false,
                       pAB2 = false,
                       autoaim = true,
               ...
               if(R.skinIndex == 6) {
                   if([30, 33, 35, 34, 38, 40].includes(damage) && W.filter(e => fgd2(e, R) < 250 && e.sid != R.sid && (primaryReload[e.sid] == 1 || secondaryReload[e.sid] == 1) && e.primaryDamage >= 40).length > 0 && appleh == 0) {
                       heal("max", 1, 1);//anti apple insta crap
                       appleh++;
                       setTimeout(() => {
                           appleh--;
                       }, 800);
               ...
               if(R.skinIndex == 7) {
                   if([42, 46, 49, 47, 51, 55].includes(damage) && W.filter(e => fgd2(e, R) < 250 && e.sid != R.sid && (primaryReload[e.sid] == 1 || secondaryReload[e.sid] == 1) && e.primaryDamage >= 40).length > 0 && antibullh == 0) {
                       instaheal("max");//anti anti bull
                       antibullh++;
                       setTimeout(() => {
                           antibullh--;
                       }, 800);
               ...
               if(W.filter(e => damage >= 40 && fgd2(e, R) < 250 && e.sid != R.sid && (primaryReload[e.sid] == 1 || secondaryReload[e.sid] == 1) && e.primaryDamage >= 40).length > 0 && normalh == 0) {
                   instaheal("max");
                   normalh++;
                   setTimeout(() => {
                       normalh--;
                   }, 800);
               }else {//anti insta and auto heal ezezez
                   heal("max", 100, 0);
               }
           }
       }
       ...
       var buybutbut=0, millY = 0, millX = 0;
       let chicken_is_too_good = setInterval(() => {
           Oe.innerText >= hatsAcc[buybutbut].cost && ("acc" == hatsAcc[buybutbut].type ? (r.send("c", 1, hatsAcc[buybutbut].id, 1), hatsAcc[buybutbut].status = true, setTimeout(() => {
               buybutbut++;
           }, 50)) : (r.send("c", 1, hatsAcc[buybutbut].id, 0), hatsAcc[buybutbut].status = true, setTimeout(() => {
               buybutbut++;
           }, 50))), 1 == hatsAcc[hatsAcc.length - 1].status && (clearInterval(chicken_is_too_good));
       }, 500);
       function attColl1() {
           return N.some(e => arrayDis(e, R) < fgd(nEy, R) && (Math.abs(Math.atan2(e.y-R.y, e.x-R.x) - nEA) <= e.scale/arrayDis(e, R)) && e.active && !e.ignoreCollision)
       }
       ...
       setInterval(() => {
           statusM = "none";
           if(R && R.alive) {
               if(autoaim == true) r.send("D", nEA), r.send("D", nEA);
               if(breakingBuild == false && R.weapons[0] != 8 && !autoaim && pAB2 == false && pAB == false) {
                   if(!InstaVarible && nEy.length && (fgd(nEy, R)/1.56) < l.weapons[R.weapons[0]].range && intrap==false && secondaryReload[R.sid] == 1 && nEy[9] != 26) {
                       let t = Ii(nEy[0]);
                       if(primaryReload[R.sid] == 1) {
                           if(nEy[9] == 11) {
                               if(checkforturrets() && R.skins[22]) {
                                   eHat(22);
                               }else {
                                   eHat(6);
                               }
                       ...
                       eAcc(21);
                       statusM = "b";
                       if(primaryReload[R.sid]) {
                           Sn(R.weapons[0], 1);
                           r.send("F", 1, nEA), r.send("F", 0, nEA);
                       }
                       r.send("D", nEA), r.send("D", nEA);
                   }
               }
               if(getElement('autoGrindez').checked && breakingBuild==false && !InstaVarible) {
                   statusM = "ag";
                   if(R.weaponIndex != R.weapons[0]) {
                       weaponON = R.weapons[1];
                       if(secondaryReload[R.sid] == 1) {
                           eHat(40);
                           r.send("F", 1, Number.MAX_VALUE), r.send("F", 0, Number.MAX_VALUE);
                       }else {
                           eHat(6);
                       }
                   }else {
               ...
               intrap=N.some(i => i.name == "pit trap" && i.owner.sid != R.sid && !teamDetect(i.owner.sid) && Math.hypot(i.y-R.y2, i.x-R.x2) <= 60 && i.active && getElement('autoBREAK').checked==true && autoaim == false);
               N.filter(i => i.name == "pit trap" && i.owner.sid != R.sid && !teamDetect(i.owner.sid) && Math.hypot(i.y-R.y2, i.x-R.x2) <= 60 && i.active).forEach(a => {
                   if(getElement('autoBREAK').checked==true && autoaim == false) {
                       statusM = "ab";
                       trapAngle=Math.atan2((a.y - R.y2), (a.x - R.x2));
                   }
               });
               function stopHitBreak() {
                   if(nEy.length && fgd(nEy, R)/1.56 < l.weapons[R.weapons[0]].range && InstaVarible) {
                       if(R.weapons[1] == 10 || R.weapons[1] == 11) {
                           return false
                       }else {
                           if(secondaryReload[R.sid] == 1) {
                               if(R.weapons[1]) {
               ...
               if(intrap && getElement('autoBREAK').checked==true && !stopHitBreak()) {
                   let nearSpike=N.filter(e=>e.dmg&&e.dmg>0&&e.owner.sid!=R.sid&&!teamDetect(e.owner.sid)&&Math.hypot(e.y-R.y2,e.x-R.x2)<130&&1==e.active);
                   if(nearSpike.length && nearSpike.length > 0 && nEy.length) {
                       if(R.weapons[1] == 10) {
                           if(secondaryReload[R.sid] == 1) {
                               r.send("F", 1, trapAngle), r.send("F", 0, trapAngle);
                           }
                       }else {
                           if(primaryReload[R.sid] == 1) {
                               r.send("F", 1, trapAngle), r.send("F", 0, trapAngle);
                       ...
                       r.send("D", trapAngle), r.send("D", trapAngle);
                   }else {
                       if(R.weapons[1] == 10) {
                           if(secondaryReload[R.sid] == 1) {
                               r.send("F", 1, trapAngle), r.send("F", 0, trapAngle);
               ...
               if(intrap && !autoaim && getElement('autoBREAK').checked==true) {
                   let nearSpike=N.filter(e=>e.dmg&&e.dmg>0&&e.owner.sid!=R.sid&&!teamDetect(e.owner.sid)&&Math.hypot(e.y-R.y2,e.x-R.x2)<130&&1==e.active);
                   let eTc = Ii(nEy[0]);
                   if(R.weapons[1] == 10) {
                       weaponON = R.weapons[1];
                       ...
                       if(secondaryReload[R.sid] == 1) {
                           eHat(40);
                       }else if(nearSpike.length && nearSpike.length > 0 && nEy.length && R.skins[26]) {
                           eHat(26);
                       }else if(checkforturrets() && R.skins[22]) {
                           eHat(22);
                       }else if(nEy.length && fgd(nEy, R) < 150 && eTc.primaryDamage >= 44 && R.skins[26]) {
                           eHat(26);
                       }else {
                           eHat(6);
                       }
       ...
       var turretsN = N.filter(i => i.name == "turret" && Math.hypot(i.y-R.y2, i.x-R.x2) < 600 && i.active && i.owner.sid != R.sid && !teamDetect(i.owner.sid));
       ...
       function autoQd() {
           if(window.pingTime > 99 && getElement("qisop").checked && R.clownCounter < 4) {
               Sn(R.items[0]), r.send("F", 1, pn()), r.send("F", 0, pn()), Sn(weaponON, 1);
           }
       }
       function canTick() {
           let t = Ii(nEy[0]);
           if(t && nEy.length && fgd(nEy, R)/1.56 > l.weapons[t.primary].range) {
               if(R.clownCounter > 2 && Date.now() - R.bullTick > 500) {
                   return true
               }else {
                   return false
               }
               ...
               if(!t && !nEy.length) {
                   if(R.clownCounter > 0 && Date.now() - R.bullTick > 500) {
                       return true
                   }else {
                       return false
                   }
       ...
       var priload = [], lD = 10, pAB = false, pAB2 = false;
       function pAB_() {
           let e = W.filter(i => (priload[i.sid] != primaryReload[i.sid]) && i.sid != R.sid && !teamDetect(i.sid))
           let p = e.filter(i => primaryReload[i.sid] == 1 && (fgd2(i, R) < 260) && i.primaryVar == 0 && i.primaryDamage >= 40)
           if(p.length && !canTick() && !checkforturrets() && R.primaryDamage >= 40){
               pAB = true;
           } else {
               pAB = false;
           }
           let et = Ii(nEy[0]);
           if(nEy.length && lD > 180 && fgd(nEy, R) <= 180 && !canTick() && !checkforturrets() && et.primaryVar == 0 && R.primaryDamage >= 40 && et.primaryDamage >= 40) {
               pAB2 = true
               setTickout(() => {
                   pAB2 = false
               }, 2);
           }
           (nEy.length) && (lD = (fgd(nEy, R)))
           priload = primaryReload
       }
       function doInstaStuff(type) {
           if(type == "reverse") {
               autoaim = true;
               weaponON = R.weapons[1];
               Sn(R.weapons[1], 1);
       ...
       var lastAnglePlayerX = [], lastAnglePlayerY = [], nowieX = [], nowieY = [];
       function Ti(e) {
           tickNum++;
           nEs = [];
           nEy = [];
           ...
           nEA = 0;
           if(wasdMills) {
           autoMillVar.w || !autoMillVar.a || autoMillVar.s || autoMillVar.d ?
               !autoMillVar.w && autoMillVar.a && autoMillVar.s && !autoMillVar.d ?
               autoMillVar.aim = -0.77 : autoMillVar.w || autoMillVar.a || !autoMillVar.s || autoMillVar.d ?
               !autoMillVar.w && !autoMillVar.a && autoMillVar.s && autoMillVar.d ?
               autoMillVar.aim = -2.34 : autoMillVar.w || autoMillVar.a || autoMillVar.s || !autoMillVar.d ?
               autoMillVar.w && !autoMillVar.a && !autoMillVar.s && autoMillVar.d ?
               autoMillVar.aim = 2.35 : !autoMillVar.w || autoMillVar.a || autoMillVar.s || autoMillVar.d ?
               autoMillVar.w && autoMillVar.a && !autoMillVar.s && !autoMillVar.d &&
               (autoMillVar.aim = 0.77) : autoMillVar.aim = 1.57 : autoMillVar.aim = 3.14 : autoMillVar.aim = -1.57 : autoMillVar.aim = 0;}
           if(Math.sqrt(Math.pow((autoMillVar.posy - (R.y || R.preY)), 2) + Math.pow((autoMillVar.posx - (R.x || R.preX)), 2)) > 100) {
               if(wasdMills) {
                   place(R.items[3], autoMillVar.aim + toRad(0 + 0xe54324dc82));//
                   place(R.items[3], autoMillVar.aim - toRad(72 + 0xe54324dc82));place(R.items[3], autoMillVar.aim + toRad(72 + 0xe54324dc82));
                   r.send("D", pn());
               }
               autoMillVar.posx = R.x, autoMillVar.posy = R.y;
           }
           ...
           for (n = 0; n < e.length; )
               (_ = Ii(e[n])) && (_.t1 = void 0 === _.t2 ? t : _.t2,
                                  ...
                                  nowieX[_.sid] = _.x2,
                                  nowieY[_.sid] = _.y2,
                                  (lastAnglePlayerX[_.sid] && lastAnglePlayerY[_.sid] && (tickNum - 1 == _.lastTick ? (_.preX = (_.x2 + _.x2 - lastAnglePlayerX[_.sid]), _.preY = (_.y2 + _.y2 - lastAnglePlayerY[_.sid])) : (_.preX = _.x2, _.preY = _.y2))),
                                  _.lastTick = tickNum,
                                  _.skinIndex = e[n + 9],
                                  _.tailIndex = e[n + 10],
                                  _.iconIndex = e[n + 11],
           ...
           if(nEy.length) {
               let ffs = Ii(nEy[0]);
               nEA = Math.atan2((ffs.preY || ffs.y2) - R.y2, (ffs.preX || ffs.x2) - R.x2);
           }else {
               nEA = R.dir;
           }
           ...
           if (tickQueue[tickNum]) {
               tickQueue[tickNum].forEach(e => e());
           }
           if(getElement("predictAB").checked && R.skins[11] && R.tails[21]) {
               pAB_();
           }
           lastAnglePlayerX = nowieX;
           lastAnglePlayerY = nowieY;
           nearEs.all = 0;
           if(R.secondary != R.weapons[1] && R.weapons[1]) {
               R.secondary = R.weapons[1];
           }
           if(R.primary != R.weapons[0] && R.weapons[1]) {
               R.secondary = R.weapons[1];
           }
           N.filter(t => nEy.length && ["spikes", "poison spikes", "greater spikes","spinning spikes"].includes(t.name) && t.owner.sid != nEy[0] && Math.hypot(t.y-nEy[2], t.x-nEy[1]) < 120).forEach(e => {
               if(nearEs.all) {
                   let tv = Math.hypot(e.y-nEy[2], e.x-nEy[1]), ttv = Math.hypot(nearEs.y-nEy[2], nearEs.x-nEy[1]);
                   if(tv < ttv) {
                       nearEs.x = e.x, nearEs.y = e.y, nearEs.angle = Math.atan2(e.y-nEy[2], e.x-nEy[1]);
                   }
               }else{
                   nearEs.all = 1, nearEs.x = e.x, nearEs.y = e.y, nearEs.angle = Math.atan2(e.y-nEy[2], e.x-nEy[1]);
               }
           });
           let pit_ = N.filter(i => i.name == "pit trap" && i.active);
           let wET = pit_.some(i => nEy.length && Math.hypot(i.y - nEy[2], i.x - nEy[1]) < 70);
           if(autoPushToggle && nEs.length && nearEs.all && fgd(nEy, R) > 40 && wET) {
               let e = (nEA-nearEs.angle)+nEA;
               r.send('9', e);
           }else if(autoPushToggle && fgd(nEy, R) <= 40 && nearEs.all && nEy.length && wET) {
               let e = (nEA-nearEs.angle)+nEA;
               r.send('9', e+Math.PI);
               autoPushToggle = "stop";
           }else if(autoPushToggle == "stop") {
               autoPushToggle = false;
               r.send('9', null);
           }
           turretsN = N.filter(i => i.name == "turret" && Math.hypot(i.y-R.y2, i.x-R.x2) <= 700 && i.active && i.owner.sid != R.sid && !teamDetect(i.owner.sid));
           if(InstaVarible == true && nEy.length && !justHit[R.sid] && fgd(nEy, R, true) < 179) {
               if(primaryReload[R.sid] == 1 && secondaryReload[R.sid] == 1 && turretReload[R.sid] == 1 && !attColl1("insta")) {
                   InstaVarible = false;
                   if(R.weapons[0] != 4 && R.weapons[1] != 15 && R.age > 7) {
                       R.primary = 4;
                       R.secondary = 15;
                       r.send("H", 4);
                       r.send("H", 15);
                   }
                   if(R.weapons[0] == 4 && R.weapons[1] == 10) {
                       doInstaStuff("reverse");
                   }else {
                       if(nEy[9] == 11) {
                           doInstaStuff("reverse");
                       }else if(nEy[9] != 6 && nEy[9] != 22) {
                           doInstaStuff("no bull");
                       }else {
                           doInstaStuff("normal");
                       }
                   }
           ...
           if((pAB2 == true || pAB == true) && R.tails[11]) {
               HAequip.acc = 21;
           }else {
               HAequip.acc = 11;
           }
           if (R.y2 < 2400){//canTick checkforturrets
               if(checkforturrets() && R.skins[22]) {
                   HAequip.hat = 22;
           ...
           } else if (R.y2 > 6850 && R.y2 < 7550) {//31
               if(checkforturrets() && R.skins[22]) {
                   HAequip.hat = 22;
               }else if(canTick() && R.skins[7]) {
                   HAequip.hat = 7;
           ...
           if(autoaim == false && breakingBuild == false && statusM == "none" && intrap == false) {
               if(R.skinIndex != HAequip.hat) {
                   eHat(HAequip.hat);
               }
               if(R.tailIndex != HAequip.acc) {
                   eAcc(HAequip.acc);
               }
           }
           R.team && Pt.length && 1 == clearedTeam && (clearedTeam = false), !R.team && Pt.length && 0 == clearedTeam && (Pt = [], clearedTeam = true);
           if(autoaim == false && intrap == false && getElement("autoGrindez").checked == false && breakingBuild == false) {
               if(secondaryReload[R.sid] == 1) {
                   if(autoaim == false) weaponON = R.weapons[0];
                   if(R.weaponIndex != R.weapons[0]) {
                       Sn(weaponON, 1);
                   }
               }else {
                   if(autoaim == false) weaponON = R.weapons[1];
                   if(R.weaponIndex != R.weapons[1]) {
                       Sn(weaponON, 1);
                   }
               }
           }
           if(autoaim == false && intrap == false && getElement("autoPLACER").checked && nEy.length) {
               let iEP = N.some(e => e.trap && Math.hypot(e.y - nEy[2], e.x - nEy[1]) < 60 && (teamDetect(e.owner.sid) || e.owner.sid == R.sid) && e.owner);
               let buildings = N.filter(e => e.active && e.health && Math.hypot(e.y - R.y, e.x - R.x) < 120);
               if(fgd(nEy, R) < 600 && !iEP && autoaim == false && intrap == false && buildings.length > 3) {
                   for(let i = 0; i < 12; i++) {
                       let angle = toRad(i * 30) + nEA;
                       Sn(R.items[4]), r.send("F", 1, angle), r.send("F", 0, angle), Sn(weaponON, 1);
                   }
               }
               if(fgd(nEy, R) < 200 && iEP && autoaim == false && intrap == false && buildings.length > 3) {
                   for(let i = 0; i < 12; i++) {
                       let angle = toRad(i * 30) + nEA;
                       Sn(R.items[2]), r.send("F", 1, angle), r.send("F", 0, angle), Sn(weaponON, 1);
                   }
               }
           }
       }
       function doWeaponStuff(_) {
           ...
           if(_.weaponIndex < 9){
               if(_.weaponIndex == _.primary) {
                   _.primaryVar = o.weaponVariants[_.weaponVariant].id;
                   _.primaryDamage = l.weapons[_.primary].dmg * o.weaponVariants[_.weaponVariant].val;
                   if(!_.secondary) {
                       if(_.secondary == null && _.primary == (4 || 5)) {
                           _.secondary = 15;
                       }else if(_.secondary && _.primary == 0) {
                           _.secondary = null;
                       }else if(_.secondary == null && _.primary == 7) {
                           _.secondary = 10;
                       }
                   }
                   if(_.buildIndex == -1){
                       primaryReload[_.sid] = Math.min(1, primaryReload[_.sid] + 110/l.weapons[_.primary].speed);
                   }
               }else{
               ...
               if(_.weaponIndex == _.secondary){
                   ...
                   if(!_.primary && _.secondary == 10) {
                       _.primary = 7;
                   }
                   if(_.buildIndex == -1){
                       secondaryReload[_.sid] = Math.min(1, secondaryReload[_.sid] + 110/l.weapons[_.secondary].speed);
                   }
               }else{
       ...
       /*function fgd(a, b, dic){
           let c = Ii(a[0]);
           if(dic == true) {
               return Math.sqrt(Math.pow(b.y2 - a.y2, 2) + Math.pow(b.x2 - a.x2, 2))
           }else {
               return Math.sqrt(Math.pow((b.preY || a.y2) - (c.preY || c.y2), 2) + Math.pow((b.preX || a.x2) - (c.preX || c.x2), 2))
           }
       }*/
       function fgd(t, n, i) {
    return t == Ii(t[0])
    ...
    : Math.sqrt(Math.pow(n.y - t.y, 2) + Math.pow(n.x - t.x, 2));
       ...
       function fgd2(a, b, dic){
           if(dic == true) {
               let c = a;
               return Math.sqrt(Math.pow(b.y2 - a.y2, 2) + Math.pow(b.x2 - a.x2, 2))
           }else {
               return Math.sqrt(Math.pow((b.preY || b.y2) - (c.preY || c.y2), 2) + Math.pow((b.preX || b.x2) - (c.preX || c.x2), 2))
           }
       }
       function Ii(e) {
           for (var t = 0; t < W.length; ++t)
       ...
       function Pi() {
           ...
           window.pingTime = e,
               gayness == 0 && (Ie.innerText = "Ping: " + e + " ms");
       }
       function pingSocket() {
           Ai = Date.now(),
               r.send("0")
       ...
       window.requestAnimFrame = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || function(e) {
           ...
           B = Date.now(),
            ...
            be.fillRect(0, o.mapScale - o.snowBiomeTop - d, oe, ce - (o.mapScale - o.snowBiomeTop - d))),
                   ...
                   be.lineWidth = 4,
                   be.strokeStyle = "#000",
                   be.globalAlpha = (!getElement("hasGrid").checked) ? .0 : 0.06,
                   be.beginPath();
               ...
               for (be.globalAlpha = 1,be.fillStyle = "rgba(0, 0, 70, "+nightModeDay.trans+")",be.fillRect(0, 0, oe, ce),be.strokeStyle = rt,
                    a = 0; a < W.length + Y.length; ++a)
                   if ((_ = W[a] || Y[a - W.length]).visible && (10 != _.skinIndex || _ == R || _.team && _.team == R.team)) {
                       var w = (_.team ? "[" + _.team + "] " : "") + (_ == R && getElement("statusSucks").checked ? (InstaVarible ? "1" : "0")+":"+(wasdMills ? "1" : "0")+" " : "") + (_.name || "");
                       if(getElement('buildHPez').checked==true) {
                           for(let i=0;i<N.length;i++) {
                               if(N[i].active == !0 && N[i].curretHealth && Math.hypot(N[i].y-R.y2, N[i].x-R.x2) < 500 && N[i].name && N[i].scale && !(turntoPersect(N[i].curretHealth,N[i].health) == 100)) {
                                   be.textAlign = "center",be.fillStyle = (N[i].owner.sid==R.sid?"#8ecc51":(teamDetect(N[i].owner.sid)?"#edf046":"#cc5151")), be.lineJoin = "round", be.font = "20px Hammersmith One", be.lineWidth = 6, be.strokeText(turntoPersect(N[i].curretHealth,N[i].health)+"%",N[i].x - f,N[i].y - d), be.fillText(turntoPersect(N[i].curretHealth,N[i].health)+"%",N[i].x - f,N[i].y - d)
                               }
                           }
                       }
                       if ("" != w) {
                           ...
                           Rn.crosshair.isLoaded && InstaVarible && nEy.length && _.sid == nEy[0] && _.isPlayer &&
                           ...
                           _.isPlayer && (
                               ...
                               be.lineWidth = _.nameScale ? 11 : 8,
                               be.lineJoin = "round",
                               be.strokeText(_.isPlayer?getElement('clownCounter').checked==true?_.clownCounter:"":"",_.x - f + be.measureText(w).width / 2 + o.crownPad, _.y - d - _.scale - o.nameY),
                               be.fillText(_.isPlayer?getElement('clownCounter').checked==true?_.clownCounter:"":"",_.x - f + be.measureText(w).width / 2 + o.crownPad, _.y - d - _.scale - o.nameY)
                           )
                       }
                       _.health > 0 && (o.healthBarWidth,
                                        be.globalAlpha = _.healAmin,
                                        be.fillStyle = rt,
                                        ...
                                        be.fill()),
                           _.isPlayer && getElement('reloadBARSSSS').checked && ((o.reloadBarWidth,
                                                                                  be.fillStyle = rt,
                                                                                  be.roundRect(_.x - f - o.reloadBarWidth*2 - o.healthBarPad-5, _.y - d + _.scale + o.nameY - 16, 2 * o.reloadBarWidth + 2 * o.healthBarPad, 17, 8),
                                                                                  be.fill(),
                                                                                  be.fillStyle = "#a5974c",
                                                                                  be.roundRect(_.x - f - o.reloadBarWidth*2-5, _.y - d + _.scale + o.nameY + o.healthBarPad - 16, 2 * o.reloadBarWidth * primaryReload[_.sid], 17 - 2 * o.healthBarPad, 7),
                                                                                  be.fill()),
                                                                                 (o.reloadBarWidth,
                                                                                  be.fillStyle = rt,
                                                                                  be.roundRect(_.x - f - o.healthBarPad+5, _.y - d + _.scale + o.nameY - 16, 2 * o.reloadBarWidth + 2 * o.healthBarPad, 17, 8),
                                                                                  be.fill(),
                                                                                  be.fillStyle = "#a5974c",
                                                                                  be.roundRect(_.x - f+5, _.y - d + _.scale + o.nameY + o.healthBarPad - 16, 2 * o.reloadBarWidth * secondaryReload[_.sid], 17 - 2 * o.healthBarPad, 7),
                                                                                  be.fill()))
                   }
...
document.getElementById("gameName").innerText="ae71 of hes";
document.getElementById("gameName").style.fontSize = "100px";
document.getElementById("gameName").style.marginBottom = "45px";
document.getElementById("gameName").style.color = "White";
document.getElementById("gameName").style.textShadow = " green 2px 2px 10px, black -4px -4px 10px";
setInterval(() => {
setTimeout( () => {      ///#leaderboard
document.getElementById('diedText').style.color = "black";
document.getElementById('diedText').innerHTML = "Ur so bad xd";
setTimeout( () => {
document.getElementById('diedText').style.color = "black";
document.getElementById('diedText').innerHTML = "ezezez";
}, 200);
}, 200);}
 , 500);
