...
e23.addEventListener("mouseup", n2, false);
    ...
    window.addEventListener("keydown", s.checkTrusted((function(e) {
        ...
        r.send("K", 0)) : (null != R.weapons[t - 49] && !actionQueue.length) ? (Sn(R.weapons[t - 49], !0), currentWeapon = (R.weapons[t - 49])) : null != R.items[t - 49 - R.weapons.length] ? Sn(R.items[t - 49 - R.weapons.length]) : 81 == t ? place(R.items[0]) : 82 == t ? xn() : mn[t] ? bn() : 32 == t && (
        vn())),
    ...
    function doBowInsta(preTurret, bowMM, cBowMM, muskMM){
        ...
        let insta = [
            ...
            ...[
                ...
                new Action(cBowMM, null, 0, 1, 1, () => {r.send("H", 12);wn = null}),//crossbow
                new Action(muskMM, null, 0, 1, 1, () => {r.send("H", 15);wn = null}),//musket
                new Action(6, null, 1, 0, 0),//switch to primary
            ],
        ];
        if(preTurret != 0){
    ...
    async function edgePlacer(){
        ...
        await fallingEdge();
        Sn(currentWeapon, 1);
        r.send("D", currentAction.autoaim ? enemyAngle : pn());
        clearInterval(id);
    }
    function place(item, dir = pn()){
        Sn(item), r.send("F", 1, dir)/*,r.send("c", 0, dir)*/, Sn(currentWeapon, 1)
    }
    var lastWN;
    var actionQueue = []
    var placeQueue = [];
					...
					if(nearestEnemy.reload[0] == -1 && l.weapons[nearestEnemy.weaponIndex].dmg + l.weapons[R.weapons[0]].dmg >= 80 && !nearestEnemy.weaponVariant){
						if(R.clownCount < 5){//auto spike gear
                            if(nearestEnemy.health <= 82 && R.reload[0] < 0){//advanced antibull
                                currentAction = new Action(11, 21, 1, 0, 1, function(e){r.send("F", 1);wn=enemyAngle});
                                enqueue([new Action(6, 19, 1, 0, 0, function(e){r.send("F", 0);bn()})]);
                                addLog("Antibull", "Hit");
                            }else{
                                hat = 11;
                                acc = 21;
        ...
        if(autoMill && (!BOT || !nearestEnemy.x && (R.itemCounts[3]||0) < 99)){
            ...
            if(wn != null){
                ...
                if(dAng(wn, lastWN) < 0.1){
                    r.send("F", 1, wn + Math.PI*0.8);r.send("F", 0);
                    r.send("F", 1, wn - Math.PI*0.8);r.send("F", 0);
                }
                Sn(currentWeapon, 1)
            }
    ...
    class Action{
        ...
        execute(){
            ...
            if(dAng(R.d2, this.autoaim ? enemyAngle : pn()) > 0.1 || this.autoaim)r.send("D", this.autoaim ? enemyAngle : pn());
            ...
            !function(e) {
                if (R && R.alive) {
                    for(let w of R.weapons){
                        //let ctx = null;//document.getElementById("actionBarReload" + w).getContext("2d");
                        //ctx.resetTransform();
                ...
                try {
                    ...
                    this.socket.onmessage = function(e) {
                        var t = new Uint8Array(e.data)
                          ...
                          , a = s[0];
                        t = s[1],
                        "io-init" == a ? (r.socketId = t[0], window.isBOT && r.send("M", {name: null,moofoll: 1,skin: "constructor"})) : n[a].apply(void 0, t)
                    }
                    ,
